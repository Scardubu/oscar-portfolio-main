...
export function HeroSection() {
  ...
  const heroContainer = staggerContainer(0.2, 0.06);
  const cinematicBlur = !reducedMotion && isDesktopViewport;
  const child = reducedMotion
    ? noMotion
    : ({
        hidden: cinematicBlur ? { opacity: 0, y: 15, filter: 'blur(4px)' } : { opacity: 0, y: 15 },
        visible: {
          opacity: 1,
          y: 0,
          filter: cinematicBlur ? 'blur(0px)' : undefined,
          transition: {
            duration: 0.8,
