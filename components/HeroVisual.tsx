...
//   UPGRADE: SystemStatusRow — HealthRing SVG replaces plain text color spans.
//            Status ring communicates health state without implying an SLA percentage.
...
// ────────────────────────────────────────────────────────────────────────────
// HealthRing — status ring (not a percentage)
// ────────────────────────────────────────────────────────────────────────────
function HealthRing({ healthy }: Readonly<{ healthy: boolean }>) {
  const r = 9;
  const circ = 2 * Math.PI * r;
  // Fixed arc lengths so we don't imply an SLA percentage.
  // Healthy = near-full ring, Degraded = short arc.
  const dash = healthy ? circ * 0.78 : circ * 0.28;
  const gap = circ - dash;
