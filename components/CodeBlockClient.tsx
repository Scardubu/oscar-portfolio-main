...
function getLineNumberWidth(lineCount: number) {
  if (lineCount >= 100) {
    return 'w-12';
  }
...
export function CodeBlockClient({
  ...
  const lines = useMemo(() => {
    ...
    if (splitLines.length > 1 && splitLines.at(-1) === '') {
      splitLines.pop();
    }
    return splitLines;
  }, [codeText]);
  ...
  const headerGradient =
    LANGUAGE_COLORS[normalizedLanguage] || 'from-white/5 to-white/[0.02] border-white/10';
  const badgeColor =
    LANGUAGE_BADGE_COLORS[normalizedLanguage] || 'bg-white/10 text-gray-200 border-white/20';

  const handleCopy = useCallback(async () => {
    if (!codeText || !globalThis.navigator?.clipboard) {
      return;
    }

    try {
      await globalThis.navigator.clipboard.writeText(codeText);
      setCopied(true);
      trackEvent('Blog', 'CodeBlockCopy', filename || language || 'unknown');
      globalThis.setTimeout(() => setCopied(false), 2000);
    } catch {
      // Intentionally swallow clipboard failures on unsupported clients.
    }
  ...
  return (
    <div className="group my-6 overflow-hidden rounded-xl border border-white/10 bg-bg-secondary/80 shadow-lg transition-all hover:border-white/20 hover:shadow-xl">
      <div
        className={`flex items-center justify-between border-b bg-gradient-to-r px-4 py-2.5 ${headerGradient}`}
      >
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-[11px] font-semibold tracking-wider uppercase ${badgeColor}`}
          >
            <span className="h-2 w-2 rounded-full bg-current opacity-60" />
            {displayLanguage}
          </span>
          {filename ? (
            <span className="truncate font-mono text-xs text-gray-400" title={filename}>
              {filename}
            </span>
          ) : null}
        </div>
          ...
          {showLineNumbers ? (
            <code className="block font-mono">
              {lines.map((line, index) => (
                <div key={`${index}-${line}`} className="flex transition-colors hover:bg-white/[0.02]">
                  <span
                    className={`mr-4 shrink-0 select-none border-r border-white/5 pr-4 text-right text-xs text-gray-500/60 ${lineNumberWidth}`}
                    aria-hidden="true"
                  >
                    {index + 1}
                  </span>
                  <span className="flex-1 whitespace-pre text-gray-300">{line || ' '}</span>
                </div>
              ))}
            </code>
          ) : (
            <code className="block whitespace-pre font-mono text-gray-300">{codeText}</code>
          )}
        </pre>

        {lines.length > 15 ? (
          <div className="pointer-events-none absolute right-0 bottom-0 left-0 h-8 bg-gradient-to-t from-bg-secondary to-transparent" />
        ) : null}
      </div>
    </div>
  );
