...
function scanFile(filePath: string): string[] {
  ...
  // V1.3: Catch 'send the constraint' / 'describe the constraint' / 'your constraint'
  // as noun-form in rendered copy on contact/footer surfaces.
  // 'constraint' as brand positioning at hero level is intentional and allowed
  // ('That's not a slogan. It's a design constraint.'). This rule targets only
  // ContactSection.tsx and Footer.tsx where the insider noun creates conversion friction.
  // Scoped to prevent false positives on hero brand-positioning copy.
  const CONTACT_SURFACES = new Set([
    path.join(ROOT, 'components', 'ContactSection.tsx'),
    path.join(ROOT, 'components', 'Footer.tsx'),
  ]);
  if (
    CONTACT_SURFACES.has(filePath) &&
    /\b(send|describe|share|submit|tell me)\s+your\s+constraint\b/i.test(content)
  ) {
    issues.push(
      "Insider noun 'constraint' in form/CTA context — use plain English ('send it over', 'tell the story')"
    );
  }
