...
// ── PATCH v2026.16 [viewport presets] ─────────────────────────────────────
// Negative top margin (margin: '-20px 0px') shrank the IntersectionObserver
// root from the top. On iOS Safari where the dynamic URL bar collapses the
// visual viewport, this shrunk the detection zone below the actual visible
// area — animations never triggered on first scroll.
// Fix: bottom-only negative margin contracts the root 40px from the bottom,
// so the observer fires 40px *before* the element exits the screen bottom —
// natural reveal timing, and immune to dynamic URL bar height changes.
// Same pattern as the already-fixed HeroSection and ProjectsSection.
export const viewportOnce    = { once: true, amount: 0.2,  margin: '0px 0px -40px 0px' } as const; // was: amount:0.3, margin:'-20px 0px'
export const viewportRelaxed = { once: true, amount: 0.15, margin: '0px 0px -40px 0px' } as const; // was: amount:0.25, margin:'-20px 0px'
export const viewportTight   = { once: true, amount: 0.28, margin: '0px 0px -30px 0px' } as const; // was: amount:0.35, margin:'-20px 0px'
